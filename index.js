...
/*const regras = [
  ...
  {
    condicao: "Texto branco",
    filtro: (img) => img.classList.contains("txt-branco"),
  },
  {
    condicao: "Texto preto",
    filtro: (img) => img.classList.contains("txt-preto"),
  },

  {
    condicao: "Sublinhado contínuo",
    filtro: (img) => img.classList.contains("continuo"),
  },
  {
    condicao: "Sublinhado pontilhado",
    filtro: (img) => img.classList.contains("potilhado"),
  },

  {
    condicao: "Cor azul && Texto branco",
    filtro: (img) =>
      img.classList.contains("azul") && img.classList.contains("txt-branco"),
  },
  {
    condicao: "Cor vermelha || Forma círculo",
    filtro: (img) =>
      img.classList.contains("vermelho") || img.classList.contains("circulo"),
  },
  {
    condicao: "Forma quadrado && Texto preto",
    filtro: (img) =>
      img.classList.contains("quadrado") && img.classList.contains("txt-preto"),
  },
  {
    condicao: "Sublinhado pontilhado || Cor verde",
    filtro: (img) =>
      img.classList.contains("potilhado") || img.classList.contains("verde"),
  },

  { condicao: "ID > 15", filtro: (img) => parseInt(img.id) > 15 },
  { condicao: "ID < 10", filtro: (img) => parseInt(img.id) < 10 },
  {
    condicao: "ID >= 20 && Texto preto",
    filtro: (img) =>
      parseInt(img.id) >= 20 && img.classList.contains("txt-preto"),
  },
  {
    condicao: "ID <= 5 || Cor rosa",
    filtro: (img) => parseInt(img.id) <= 5 || img.classList.contains("rosa"),
  },
];*/
